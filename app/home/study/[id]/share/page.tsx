...
import { getStudyDetails, StudyDetails } from "@/lib/api/StudyAPI"
...
export default function StudySharePage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        setLoading(true)
        const data = await getStudyDetails(studyId)
        setStudy(data)
      } catch (e: any) {
  ...
  const shareUrl = useMemo(() => {
    if (!study) return ""
    // Prefer backend-provided share_url; fallback to token pattern
    if (study.share_url) return study.share_url
    if (study.share_token && typeof window !== "undefined") {
      return `${window.location.origin}/participate/${study.share_token}`
    }
    return ""
  }, [study])
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="bg-white rounded-xl shadow-sm border">
            <div className="px-5 sm:px-8 py-5 border-b" style={{ borderColor: "#E5EEF6" }}>
              <h1 className="text-xl font-semibold" style={{ color: BRAND }}>Share Study</h1>
            </div>
              ...
              <section className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="flex gap-4">
                  ...
                  <div>
                    ...
                    <a
                      href={qrSrc}
                      download={`study-${study.id}-qr.png`}
